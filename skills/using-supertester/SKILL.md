...
| 对比现有测试资产/查缺补漏 | 先 requirement-analysis / requirement-association，再进入报告或对比分析 | "对比 AI 用例和现有人工用例" |
| 查询/问答 | 直接回答 | "checkout 模块需要哪些测试？" |

如果用户提供了现有测试资产（人工用例、历史 case、测试清单、缺陷单、回归包），不要把它们只当参考材料。应优先把它们视为”现有测试资产”，**在 Phase 1 就消化吸收**，而不是等到最后阶段才对比：

**检测到现有测试资产时的处理流程：**
1. 在 Phase 1 解析需求的同时，解析现有测试资产，提取：
...
3. Phase 3 生成时，确保这些高价值资产不被遗漏
4. 生成完成后，自动与现有资产做 gap analysis，输出四象限对比：
   - AI 新增覆盖（现有资产未覆盖）
   - AI 遗漏的高价值场景（现有资产已覆盖但 AI 未覆盖）
   - 双方共有（可去重合并）
   - 双方都缺（需求盲区）
...
每个阶段的关注重点：
- **Phase 1**: 解析功能需求 + 抽取测试资产 + 明确证据类型 + 识别多语言/Prompt/测试数据资产 + 消化现有测试资产
...
| "用户催得急" | 返工成本远高于流程成本。 |
| "现有测试用例只是参考" | 现有测试资产往往包含高价值规则、内容、状态断言和经验边界，忽略它们会让新生成结果失真 |
...
当用户提供了“已有测试资产 + 新生成测试资产”这类输入时，默认按下面思路处理：

1. 先把已有测试资产纳入 Phase 1/2 的上下文，而不是只在最后报告阶段查看
2. 用统一维度比较：
   - 行为覆盖
   - 规则覆盖
   - 内容覆盖
...
不要把对比任务退化成“数量统计”或“谁更多”。重点是覆盖结构和测试资产保真度。
