...
- **价格/金额字段专用等价类（强制）**: 当字段表示价格、金额、费用、余额、折扣、税费、运费、支付、退款、充值或结算金额时，必须额外生成“展示值 vs 提交值”传输契约分区，验证前端提交给服务端的是规范化金额值，而不是格式化展示字符串。至少覆盖：
  - 千位以上展示值含分组符：页面显示 `"1,234.56"` / `"1 234,56"` / `"1.234,56"` 时，API 入参必须为服务端约定的规范值（如 `"1234.56"` 或整数分 `123456`），不得携带千分位分隔符
  - 带货币符号或币种码：显示 `"¥1,234.56"` / `"USD 1,234.56"` 时，金额入参与币种字段必须拆分正确，不得把符号或币种拼进金额字段
  - 本地化小数点/分组符：不同 locale 下逗号/点号含义不同，提交值必须按服务端契约统一规范化
  - 前后空格、不可见空格、全角符号、复制粘贴输入：提交前必须清洗或拦截
  - `0`、空值、负数、超大值、超精度小数、科学计数法（如 `1e3`）必须按金额规则处理
  - 小数精度、舍入/截断、整数分和 decimal 单位转换必须通过 API / DB / 支付网关响应共同验证
...
- **价格/金额边界（强制）**: 金额字段必须覆盖格式化阈值和精度边界，尤其是从 999.99 到 1,000.00 这种触发千分位展示的边界。至少覆盖：999 / 999.99 / 1000 / 1000.00 / 1000.01 / 最大允许金额 / 最大金额+最小单位 / 最小非零金额 / 0 / 负数 / 超过允许小数位。边界预期必须同时断言 UI 展示值、API 入参值、服务端存储值或支付网关接收值。
...
- **价格/金额传参异常（强制）**: 对支付、订单、报价、退款、结算等金额链路，必须补充 API 入参异常和支付失败兜底用例：
  - 前端误传带千分位分隔符的展示值（如 `"1,234.56"`）时，服务端应拒绝并返回明确校验错误，或前端在提交前规范化为契约值；不得进入支付失败的下游状态
  - 误传货币符号、币种码、空格、本地化格式、科学计数法、超精度小数、NaN/Infinity、负数等非法金额时，系统应拦截并保持订单/支付状态一致
  - 支付网关因金额格式或精度拒绝时，订单状态、支付记录、错误提示、重试入口和审计日志必须一致，不得出现已扣款/未下单、重复支付或状态卡死
...
1. 列出所有条件变量（因）及其完整取值
2. 列出所有结果变量（果）及其完整取值
3. 显式标注条件间约束关系：互斥、包含、唯一、要求、前置依赖、非法组合
4. 先形成因果关系摘要，再转化为判定表；不允许把互斥或非法组合当成真实业务用例生成
5. 生成条件组合矩阵
6. 每条规则（Rule）作为一条 row（最终成为分组 step 的一个 children 叶子 step）：`action` 列出该规则下所有条件取值（用 `|` block scalar + `1. 2. 3.`），`result` 列出对应动作
7. 优化：合并无差异的规则（在 `design-artifacts.md` 中说明合并依据，必要时在 group 末尾以注释或独立 row 说明）
8. **整体作为一个 partition_group 输出**

**方法产出物（必须写入 `.supertester/test-cases/design-artifacts.md`）:**
...
### 判定表
| Rule | 条件组合 | 预期结果 | 生成 row/source | 合并/裁剪理由 |
|------|----------|----------|-----------------|---------------|
```

### 正交压缩子模式（orthogonal_mode）

当条件变量 >3，且多个条件存在 >2 个水平，全组合数量过大时，可启用正交压缩子模式。但这只是决策表生成器的压缩模式，不是放弃覆盖。

触发前必须判断:
- 如果完整组合矩阵本身是需求（如完整价格矩阵、权限矩阵、合规规则表），禁止正交压缩，必须逐项覆盖或以附录保留完整矩阵
- 安全、鉴权、资金、数据完整性、不可逆操作默认禁止压缩；确需压缩时必须在 findings.md 记录用户确认或风险依据
- 外部系统契约、schema、错误码映射等“完整列表即合约”的场景禁止用代表组合替代

启用 `orthogonal_mode` 时必须在 `design-artifacts.md` 记录:
...
| 正交/精简组合 | 条件取值 | 预期结果 | 覆盖目标 |
|---------------|----------|----------|----------|

| 风险补充组合 | 补充原因 | 预期结果 |
|--------------|----------|----------|

| 未全组合覆盖范围 | 省略理由 | 风险接受依据 |
|------------------|----------|--------------|
```

输出要求:
- 正交表或精简组合中的每一行仍作为 partition_group 的 row 或独立 single/scenario_chain 输出
- 必须额外补充高风险组合，尤其是边界值、默认值、最高权限、最低权限、失败/超时/降级组合
- `deduplication-report.md` 记录压缩前组合数、压缩后 row 数、风险补充 row 数
