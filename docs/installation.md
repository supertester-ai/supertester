...
| [Codex](#codex-安装) | Marketplace 安装 | `.codex-plugin/` + `.agents/plugins/marketplace.json` + `hooks.json` + `agents/` + `skills/` | Codex Plugins |
...
先注册 Supertester 的 marketplace 源：
...
然后按你的 Codex 版本支持的本地插件安装方式安装该目录。
